...
Relaxation::~Relaxation() { delete m_space; }
  ...
  for (uint32_t y = 0; y < m_ySize; y++)
    for (uint32_t x = 0; x < m_xSize; x++) {
      if (QRandomGenerator::global()->bounded(1000) == 0) {
        // locus
        uint32_t color =
            0xff000000 + (QRandomGenerator::global()->generate() & 0xff) +
...
bool Relaxation::drawLocusEdge(Locus *locus, uint32_t len) {
  ...
  for (uint32_t i = 0; i <= len; i++) {
    ...
    if (0 <= x_l && x_l < (int32_t)m_xSize && 0 <= y_u &&
        y_u < (int32_t)m_ySize && m_space[y_u * m_xSize + x_l] == 0) {
      m_space[y_u * m_xSize + x_l] = locus->m_color;
      locus->add(y_u * m_xSize + x_l);
      changed = true;
    }
    if (0 <= x_l && x_l < (int32_t)m_xSize && 0 <= y_d &&
        y_d < (int32_t)m_ySize && m_space[y_d * m_xSize + x_l] == 0) {
      m_space[y_d * m_xSize + x_l] = locus->m_color;
      locus->add(y_d * m_xSize + x_l);
      changed = true;
    }
    if (0 <= x_r && x_r < (int32_t)m_xSize && 0 <= y_u &&
        y_u < (int32_t)m_ySize && m_space[y_u * m_xSize + x_r] == 0) {
      m_space[y_u * m_xSize + x_r] = locus->m_color;
      locus->add(y_u * m_xSize + x_r);
      changed = true;
    }
    if (0 <= x_r && x_r < (int32_t)m_xSize && 0 <= y_d &&
        y_d < (int32_t)m_ySize && m_space[y_d * m_xSize + x_r] == 0) {
